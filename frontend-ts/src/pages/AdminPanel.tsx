...
function UsersTab() {
  ...
  const handleDelete = async (u: AnyObj) => { if (!window.confirm(`Xóa user "${u.name || u.email}"?`)) return; try { await userApi.delete(u.userId as string); load() } catch (e: unknown) { const err = e as { response?: { data?: { message?: string } } }; alert(err.response?.data?.message || 'Lỗi') } }
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault(); setFormError('')
    const trimName = form.name.trim()
    if (trimName.length < 2) { setFormError('Tên phải có ít nhất 2 ký tự'); return }
    if (trimName.length > 100) { setFormError('Tên tối đa 100 ký tự'); return }
    if (!editing) {
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email.trim())) { setFormError('Email không hợp lệ'); return }
      if (form.password.length < 6) { setFormError('Mật khẩu phải có ít nhất 6 ký tự'); return }
    }
    if (form.avatarUrl && !/^https?:\/\/.+/.test(form.avatarUrl.trim())) { setFormError('Avatar URL phải bắt đầu bằng http:// hoặc https://'); return }
    setSaving(true); const roleCodes = form.roleCodes.split(',').map((s) => s.trim()).filter(Boolean); try { if (editing) { await userApi.update(editing.userId as string, { name: trimName, avatarUrl: form.avatarUrl?.trim() || undefined, isActive: true, roleCodes: roleCodes.length ? new Set(roleCodes) : undefined }) } else { await userApi.create({ email: form.email.trim(), password: form.password, name: trimName, avatarUrl: form.avatarUrl?.trim() || undefined, roleCodes: new Set(roleCodes) }) }; setShowModal(false); setPage(0); load() } catch (err: unknown) { const e = err as { response?: { data?: { message?: string } }; message?: string }; setFormError(e.response?.data?.message || e.message || 'Lỗi') }; setSaving(false) }
  ...
  return <div className={sectionCls}>
    <div className="flex items-center justify-between gap-3 mb-4 flex-wrap"><h2 className="m-0 text-xl font-bold">Quản lý Users</h2><button type="button" className={btnPrimary} onClick={openCreate}>+ Tạo User</button></div>
    {/* Search */}
    <div className="flex items-center gap-2 bg-bg-deep border border-border-medium rounded-xl px-3 py-2 mb-4 max-w-[400px]">
      ...
      <input className="flex-1 bg-transparent border-none outline-none text-text-main text-sm" placeholder="Tìm theo tên hoặc email..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
    ...
    <Modal title={editing ? 'Sửa User' : 'Tạo User'} show={showModal} onClose={() => setShowModal(false)} onSubmit={handleSubmit} error={formError} saving={saving}>
      ...
      <Label text="Roles (comma-separated)"><input className={inputCls} value={form.roleCodes} onChange={(ev) => setForm({ ...form, roleCodes: ev.target.value })} /></Label>
    </Modal>
    {loading ? <div className="p-4 text-center text-text-muted">Đang tải...</div> : <><div className="overflow-x-auto"><table className={tableCls}><thead><tr><th>Name</th><th>Email</th><th>Roles</th><th>Active</th><th>Actions</th></tr></thead><tbody>{filteredUsers.map((u) => <tr key={u.userId as string}><td>{u.name as string}</td><td>{u.email as string}</td><td>{((u.roles || []) as AnyObj[]).map((r) => (r.roleName || r.roleCode || r) as string).join(', ')}</td><td><Badge status={u.active || u.isActive ? 'Active' : 'Inactive'} /></td><td><div className="flex gap-1.5 flex-wrap"><button type="button" className={`${btnGhost} ${btnSm}`} onClick={() => openEdit(u)}>Sửa</button><button type="button" className={`${btnGhost} ${btnSm} ${btnDanger}`} onClick={() => handleDelete(u)}>Xóa</button></div></td></tr>)}{filteredUsers.length === 0 && <tr><td colSpan={5} className="p-4 text-center text-text-muted">{searchQuery ? 'Không tìm thấy user phù hợp' : 'Không có user'}</td></tr>}</tbody></table></div><Pagination page={page} totalPages={totalPages} onPrev={() => setPage(page - 1)} onNext={() => setPage(page + 1)} /></>}
  </div>
}
...
function RolesTab() {
  ...
  const handleDeleteRole = async (r: AnyObj) => { if (!window.confirm(`Xóa role "${r.roleName}"?`)) return; try { await roleApi.delete(r.roleCode as string); load() } catch (e: unknown) { alert((e as { response?: { data?: { message?: string } } }).response?.data?.message || 'Lỗi') } }
  const handleDeletePrivilege = async (p: AnyObj) => { if (!window.confirm(`Xóa privilege "${p.privilegeName}"?`)) return; try { await privilegeApi.delete(p.privilegeCode as string); load() } catch (e: unknown) { alert((e as { response?: { data?: { message?: string } } }).response?.data?.message || 'Lỗi') } }
  ...
  return <div className={sectionCls}>
    ...
    <div className="flex items-center justify-between gap-3 mb-4"><h2 className="m-0 text-xl font-bold">Roles</h2><button type="button" className={btnPrimary} onClick={() => openRole()}>+ Tạo Role</button></div>
    {loading ? <div className="p-4 text-center text-text-muted">Đang tải...</div> : <div className="overflow-x-auto"><table className={tableCls}><thead><tr><th>Code</th><th>Name</th><th>Privileges</th><th>Actions</th></tr></thead><tbody>{roles.map((r) => <tr key={r.roleCode as string}><td>{r.roleCode as string}</td><td>{r.roleName as string}</td><td>{((r.privileges || []) as AnyObj[]).map((p) => (p.privilegeName || p.privilegeCode) as string).join(', ') || '-'}</td><td><div className="flex gap-1.5"><button type="button" className={`${btnGhost} ${btnSm}`} onClick={() => openRole(r)}>Sửa</button><button type="button" className={`${btnGhost} ${btnSm} ${btnDanger}`} onClick={() => handleDeleteRole(r)}>Xóa</button></div></td></tr>)}{roles.length === 0 && <tr><td colSpan={4} className="p-4 text-center text-text-muted">Không có role</td></tr>}</tbody></table></div>}
    <div className="flex items-center justify-between gap-3 mb-4 mt-8"><h2 className="m-0 text-xl font-bold">Privileges</h2><button type="button" className={btnPrimary} onClick={() => openPrivilege()}>+ Tạo Privilege</button></div>
    {!loading && <div className="overflow-x-auto"><table className={tableCls}><thead><tr><th>Code</th><th>Name</th><th>Description</th><th>Actions</th></tr></thead><tbody>{privileges.map((p) => <tr key={p.privilegeCode as string}><td>{p.privilegeCode as string}</td><td>{p.privilegeName as string}</td><td>{(p.description || '-') as string}</td><td><div className="flex gap-1.5"><button type="button" className={`${btnGhost} ${btnSm}`} onClick={() => openPrivilege(p)}>Sửa</button><button type="button" className={`${btnGhost} ${btnSm} ${btnDanger}`} onClick={() => handleDeletePrivilege(p)}>Xóa</button></div></td></tr>)}{privileges.length === 0 && <tr><td colSpan={4} className="p-4 text-center text-text-muted">Không có privilege</td></tr>}</tbody></table></div>}
  </div>
}
...
function SubscriptionsTab() {
  ...
  return <div className={sectionCls}>
    <div className="flex items-center justify-between gap-3 mb-4"><h2 className="m-0 text-xl font-bold">Subscriptions</h2></div>
    <div className="flex gap-3 flex-wrap items-end mb-4"><label className="flex flex-col gap-1 text-[0.82rem] font-semibold">Status<select className={inputCls} value={filter.statusPayment} onChange={(e) => { setFilter({ ...filter, statusPayment: e.target.value }); setPage(0) }}><option value="">Tất cả</option><option value="SUCCESS">SUCCESS</option><option value="PENDING">PENDING</option><option value="ERROR">ERROR</option></select></label><label className="flex flex-col gap-1 text-[0.82rem] font-semibold">Từ ngày<input className={inputCls} type="date" value={filter.from} onChange={(e) => { setFilter({ ...filter, from: e.target.value }); setPage(0) }} /></label><label className="flex flex-col gap-1 text-[0.82rem] font-semibold">Đến ngày<input className={inputCls} type="date" value={filter.to} onChange={(e) => { setFilter({ ...filter, to: e.target.value }); setPage(0) }} /></label></div>
    {loading ? <div className="p-4 text-center text-text-muted">Đang tải...</div> : <><div className="overflow-x-auto"><table className={tableCls}><thead><tr><th>ID</th><th>Ngày</th><th>Giá</th><th>Status</th><th>PayType</th><th>Content</th></tr></thead><tbody>{subs.map((s) => <tr key={s.subcriptionId as string}><td className="text-[0.75rem]">{String(s.subcriptionId).slice(0, 8)}...</td><td>{fmtDate(s.createdAt)}</td><td>{fmtPrice(s.subcriptionPrice)}</td><td><Badge status={(s.statusPayment || '') as string} /></td><td>{(s.payType || '-') as string}</td><td className="max-w-[200px] overflow-hidden text-ellipsis whitespace-nowrap">{(s.content || s.message || '-') as string}</td></tr>)}{subs.length === 0 && <tr><td colSpan={6} className="p-4 text-center text-text-muted">Không có subscription</td></tr>}</tbody></table></div><Pagination page={page} totalPages={totalPages} onPrev={() => setPage(page - 1)} onNext={() => setPage(page + 1)} /></>}
  </div>
}
...
function ReportTab() {
  ...
  const fetchReport = async () => { if (!filter.from || !filter.to) { setError('Vui lòng chọn khoảng thời gian'); return }; if (new Date(filter.from) > new Date(filter.to)) { setError('Khoảng thời gian không hợp lệ'); return }; setLoading(true); setError(''); setReport(null); setSourceNote(''); try { const r = await buildReport({ from: filter.from, to: filter.to }); setReport(r); setSourceNote('Nguồn dữ liệu: subscriptions/search.') } catch (e: unknown) { const err = e as { response?: { data?: { message?: string; errorCode?: string } }; message?: string }; setError(err.response?.data?.message || err.response?.data?.errorCode || err.message || 'Lỗi') }; setLoading(false) }
  ...
  return <div className={sectionCls}>
    <div className="flex items-center justify-between gap-3 mb-4"><h2 className="m-0 text-xl font-bold">Báo cáo doanh thu</h2></div>
    <div className="flex gap-3 flex-wrap items-end mb-4"><label className="flex flex-col gap-1 text-[0.82rem] font-semibold">Từ ngày<input className={inputCls} type="date" value={filter.from} onChange={(e) => setFilter({ ...filter, from: e.target.value })} /></label><label className="flex flex-col gap-1 text-[0.82rem] font-semibold">Đến ngày<input className={inputCls} type="date" value={filter.to} onChange={(e) => setFilter({ ...filter, to: e.target.value })} /></label><button type="button" className={`${btnPrimary} self-end`} onClick={fetchReport} disabled={loading}>{loading ? 'Đang tải...' : 'Xem báo cáo'}</button></div>
...
function EnrollmentsTab() {
  ...
  return <div className={sectionCls}>
    <div className="flex items-center justify-between gap-3 mb-4"><h2 className="m-0 text-xl font-bold">Enrollments</h2></div>
    <div className="flex gap-3 flex-wrap items-end mb-4"><label className="flex flex-col gap-1 text-[0.82rem] font-semibold">Tìm kiếm<input className={inputCls} value={filter.keysearch} onChange={(e) => setFilter({ ...filter, keysearch: e.target.value })} placeholder="Tên khóa học..." onKeyDown={(e) => { if (e.key === 'Enter') { setPage(0); load() } }} /></label><label className="flex flex-col gap-1 text-[0.82rem] font-semibold">Status<select className={inputCls} value={filter.statusCourse} onChange={(e) => { setFilter({ ...filter, statusCourse: e.target.value }); setPage(0) }}><option value="">Tất cả</option><option value="NOT_STARTED">NOT_STARTED</option><option value="IN_PROGRESS">IN_PROGRESS</option><option value="COMPLETED">COMPLETED</option></select></label><button type="button" className={`${btnPrimary} self-end`} onClick={() => { setPage(0); load() }}>Tìm</button></div>
    {loading ? <div className="p-4 text-center text-text-muted">Đang tải...</div> : <><div className="overflow-x-auto"><table className={tableCls}><thead><tr><th>Khóa học</th><th>Ngày đăng ký</th><th>Status</th><th>Actions</th></tr></thead><tbody>{enrollments.map((e) => <tr key={e.enrollmentId as string}><td>{((e.courseResponse as AnyObj)?.title || '-') as string}</td><td>{fmtDate(e.enrollmentDate)}</td><td><Badge status={(e.statusCourse || '') as string} /></td><td><select className="bg-bg-deep border border-border-medium rounded-lg text-text-main px-2 py-1 text-[0.82rem] outline-none" value={(e.statusCourse || '') as string} onChange={(ev) => handleStatusChange(e, ev.target.value)}><option value="NOT_STARTED">NOT_STARTED</option><option value="IN_PROGRESS">IN_PROGRESS</option><option value="COMPLETED">COMPLETED</option></select></td></tr>)}{enrollments.length === 0 && <tr><td colSpan={4} className="p-4 text-center text-text-muted">Không có enrollment</td></tr>}</tbody></table></div><Pagination page={page} totalPages={totalPages} onPrev={() => setPage(page - 1)} onNext={() => setPage(page + 1)} /></>}
  </div>
}
...
const AdminPanel = () => {
  ...
  return (
    <div className="min-h-screen bg-bg-page text-text-main">
      <Header />
      <div className="bg-[linear-gradient(135deg,#1e1b4b_0%,#312e81_50%,#4338ca_100%)] px-6 py-8 text-white"><div className="max-w-[1280px] mx-auto"><div className="flex items-center gap-1.5 text-[0.82rem] mb-2 text-white/65"><a href="/" className="text-white/85 no-underline hover:underline">Trang chủ</a><span>/</span><span>Admin</span></div><h1 className="m-0 text-2xl font-extrabold">🛡️ Admin Dashboard</h1><p className="mt-1 mb-0 text-white/70 text-sm">Quản lý người dùng, quyền hạn, giao dịch và báo cáo hệ thống.</p></div></div>
