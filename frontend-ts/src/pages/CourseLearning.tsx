...
const CourseLearning = () => {
  ...
  const findRealContentId = (ct: AnyObj | null) => {
    ...
    // 1. Lấy ID gốc (loại bỏ tiền tố quiz- hoặc doc- nếu có)
    const rawId = getContentId(ct).replace(/^(quiz|doc)-/, '');

    // 2. Nếu là UUID chuẩn, dùng luôn
    if (isUuid(rawId)) return rawId;

    // 3. Tìm kiếm trong danh sách gốc (Dữ liệu chưa bị normalize)
    const contents = contentsByLesson[selectedLessonId] || [];
    const real = contents.find((c) => {
      const item = c as AnyObj;
      // So sánh ID gốc từ API với ID đã lọc tiền tố
      return item.contentType === ct.contentType &&
        (item.contentId === rawId || item.examId === rawId);
    });

    return real ? (real.examId || real.contentId) : rawId;
  };
      ...
      <main className="w-full mx-auto pb-12">
        <div className={`grid ${sidebarOpen ? 'grid-cols-[340px_minmax(0,1fr)]' : 'grid-cols-[0px_minmax(0,1fr)]'} relative transition-all duration-[350ms] ease-[cubic-bezier(0.4,0,0.2,1)]`}>
          ...
          <section className="px-6 py-5 flex flex-col gap-4 min-h-[calc(100vh-64px)]">
            ...
            {cpct >= 100 && certDone && <div className="flex items-center justify-between gap-3 bg-blue-50 border border-blue-200 rounded-2xl px-4 py-3 font-semibold flex-wrap"><span>{t('learning.certCreated', { name: displayLearnerName })}</span><div className="flex gap-2"><button type="button" className="px-3 py-2 rounded-xl font-bold border border-blue-300 cursor-pointer bg-white text-blue-600 no-underline text-[0.85rem] transition-colors hover:bg-blue-100" onClick={() => { const url = `${window.location.origin}/verify-certificate?courseId=${courseId}&learnerId=${user?.userId}`; navigator.clipboard.writeText(url).then(() => { const btn = document.activeElement as HTMLButtonElement; if (btn) { btn.textContent = '✅ Đã sao chép!'; setTimeout(() => { btn.textContent = '📋 Sao chép link' }, 2000) } }) }}>📋 Sao chép link</button><Link to="/my-certificates" className="px-4 py-2 rounded-xl font-bold border-none cursor-pointer bg-green-600 text-white no-underline text-[0.88rem] transition-colors hover:bg-green-700">{t('learning.viewCerts')}</Link></div></div>}
