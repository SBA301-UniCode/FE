...
const CourseLearning = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!enriched.some((ct) => (ct as AnyObj).contentType === 'DOCUMENT')) enriched.push(normalizeContent({ contentId: `doc-${selectedLessonId}`, contentType: 'DOCUMENT', lessonId: selectedLessonId, _virtual: true }))
        if (!enriched.some((ct) => (ct as AnyObj).contentType === 'QUIZ')) enriched.push(normalizeContent({ contentId: `quiz-${selectedLessonId}`, contentType: 'QUIZ', lessonId: selectedLessonId, _virtual: true }))
  ...
  const handleMarkDocRead = () => { const cid = getContentId(selectedContent); if (isTrackableContentId(cid)) { setDocRead(true); trackContent(cid, 'COMPLETED') } }
  ...
  useEffect(() => { let c = false; const load = async () => { if (selectedContent?.contentType !== 'PRACTICE') { setPracticeError(''); setPracticeSession(null); setPracticeCode(''); setPracticeResult(null); setPracticeSubmitError(''); return }; const cid = getContentId(selectedContent); if (!isTrackableContentId(cid)) { setPracticeError('Không tìm thấy contentId hợp lệ.'); return }; setPracticeLoading(true); setPracticeError(''); try { const res = await practiceApi.startPractice(cid); if (c) return; const pp = unwrap(res) as AnyObj; setPracticeSession(pp || null); setPracticeCode(extractPracticeStarterCode(pp)); setPracticeResult(null); setPracticeSubmitError(''); trackContent(cid, 'IN_PROCESS') } catch (err: unknown) { if (c) return; setPracticeSession(null); setPracticeCode(''); const e = err as { response?: { data?: { message?: string } }; message?: string }; setPracticeError(e.response?.data?.message || e.message || 'Không thể tải bài thực hành.') } finally { if (!c) setPracticeLoading(false) } }; load(); return () => { c = true } }, [selectedContent])

  const handleSubmitPractice = async () => { if (!practiceSession?.submissionId) { setPracticeSubmitError('Thiếu submissionId.'); return }; setSubmittingPractice(true); setPracticeSubmitError(''); try { const res = await practiceApi.submitPractice({ submissionId: practiceSession.submissionId as string, learnerCode: practiceCode }); const pp = unwrap(res) as AnyObj; setPracticeResult(pp || null); const scid = getContentId(selectedContent); if (isTrackableContentId(scid) && Number(pp?.failed || 0) === 0) trackContent(scid, 'COMPLETED') } catch (err: unknown) { const e = err as { response?: { data?: { message?: string } }; message?: string }; setPracticeSubmitError(e.response?.data?.message || e.message || 'Nộp bài thất bại.'); setPracticeResult(null) } finally { setSubmittingPractice(false) } }

  const getChapterTitle = (c: AnyObj) => (c?.title ?? c?.chapterTitle ?? 'Chương') as string
  const getLessonTitle = (l: AnyObj) => (l?.title ?? l?.lessonTitle ?? 'Bài giảng') as string
    ...
    <div className="min-h-screen bg-bg-page text-text-main">
      ...
      <main className="max-w-[1400px] mx-auto pb-12">
        <div className={`grid ${sidebarOpen ? 'grid-cols-[340px_minmax(0,1fr)]' : 'grid-cols-[0px_minmax(0,1fr)]'} relative transition-all duration-[350ms] ease-[cubic-bezier(0.4,0,0.2,1)]`}>
          ...
          {/* Toggle */}
          <button type="button" className={`fixed top-20 z-[1100] w-10 h-10 rounded-xl border border-border-medium bg-white text-text-secondary text-lg cursor-pointer flex items-center justify-center transition-all shadow-[0_2px_8px_rgba(0,0,0,0.08)] hover:bg-[rgba(0,86,210,0.06)] hover:border-primary-500 hover:text-primary-500 ${sidebarOpen ? 'left-[300px]' : 'left-4'}`} onClick={() => setSidebarOpen((v) => !v)} title={sidebarOpen ? 'Đóng menu' : 'Mở menu'}>{sidebarOpen ? '✕' : '☰'}</button>
          ...
          <aside className={`bg-white border-r border-border-medium h-[calc(100vh-64px)] sticky top-16 overflow-hidden transition-all duration-[350ms] ease-[cubic-bezier(0.4,0,0.2,1)] flex flex-col ${sidebarOpen ? 'w-[340px] min-w-[340px]' : 'w-0 min-w-0 border-r-0 p-0'}`}>
            <div className="flex justify-between items-center px-5 pt-4 pb-2 shrink-0"><h2 className="m-0 text-base font-bold whitespace-nowrap">Nội dung khóa học</h2><button type="button" className="bg-transparent border-none text-text-muted text-lg cursor-pointer p-0.5 rounded-md hover:text-text-main hover:bg-bg-deep" onClick={() => setSidebarOpen(false)}>✕</button></div>
            {loading && <div className="px-5 py-4 text-[0.88rem] text-text-muted whitespace-nowrap">Đang tải...</div>}
          ...
          <section className="px-6 py-5 flex flex-col gap-4 min-h-[calc(100vh-64px)]">
            ...
            {/* Certificate banners */}
            {cpct >= 100 && certChecked && !certDone && <div className="flex items-center justify-between gap-3 bg-emerald-50 border border-emerald-200 rounded-2xl px-4 py-3 font-semibold flex-wrap"><span>{certLoading ? 'Đã hoàn thành 100%. Đang cấp chứng chỉ...' : 'Đã hoàn thành 100%. Đang chuẩn bị chứng chỉ.'}</span></div>}
            {cpct >= 100 && certDone && <div className="flex items-center justify-between gap-3 bg-blue-50 border border-blue-200 rounded-2xl px-4 py-3 font-semibold flex-wrap"><span>Chứng chỉ của {displayLearnerName} đã được tạo!</span><Link to="/my-certificates" className="px-4 py-2 rounded-xl font-bold border-none cursor-pointer bg-green-600 text-white no-underline text-[0.88rem] transition-colors hover:bg-green-700">Xem chứng chỉ</Link></div>}

            {!selectedLessonId && <div className="text-sm text-text-muted py-4">Chọn một bài giảng để bắt đầu học.</div>}
            {selectedLessonId && !selectedContent && currentContents.length === 0 && <div className="text-sm text-text-muted py-4">Bài giảng này chưa có nội dung nào.</div>}

            {/* VIDEO */}
            {selectedContent?.contentType === 'VIDEO' && <div className="flex flex-col gap-2">{currentVideo ? (getVideoUrl(currentVideo) ? <><div className="w-full bg-slate-950 rounded-2xl overflow-hidden border border-border-medium"><HlsCourseVideoPlayer src={getVideoUrl(currentVideo)} playbackVideoId={(currentVideo.videoId || currentVideo.id) as string} playbackDuration={currentVideo.playbackDuration as number} className="" onPlay={handleVideoPlay} onEnded={handleVideoEnded} /></div><div className="font-semibold text-base">Video bài giảng (HLS){currentVideo.duration ? ` · ${currentVideo.duration} phút` : ''}</div></> : <div className="text-sm text-text-muted py-4">Video chưa có URL hợp lệ.</div>) : <div className="text-sm text-text-muted py-4">Video chưa được upload cho nội dung này.</div>}</div>}

            {/* DOCUMENT */}
            {selectedContent?.contentType === 'DOCUMENT' && <div className="bg-white border border-border-medium rounded-[18px] overflow-hidden"><div className="flex items-center gap-2.5 px-4 py-3 bg-bg-deep border-b border-border-subtle"><span className="text-xl">📄</span><h3 className="m-0 text-lg font-bold">{(selectedDoc?.title as string) || 'Tài liệu bài giảng'}</h3></div><div className="px-4 py-5 text-text-secondary leading-relaxed text-sm">{(selectedDoc?.documentUrl) ? <><a href={selectedDoc.documentUrl as string} target="_blank" rel="noreferrer">📄 Mở tài liệu</a><button type="button" className="ml-3 bg-[linear-gradient(135deg,#6366f1,#8b5cf6)] border-none text-white px-4 py-2 rounded-lg cursor-pointer text-[0.85rem]" onClick={async () => { try { const res = await watermarkApi.downloadWithWatermark((selectedDoc as AnyObj).documentId as string); const blob = new Blob([(res as { data: BlobPart }).data]); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; let fn = (selectedDoc?.title || 'document') as string; const cd = ((res as { headers?: Record<string, string> }).headers || {})['content-disposition']; if (cd) { const m = cd.match(/filename="?([^"]+)"?/); if (m?.[1]) fn = m[1] } else if (!fn.includes('.')) fn += '.pdf'; a.download = fn; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url) } catch (e: unknown) { const err = e as { response?: { data?: { message?: string } }; message?: string }; alert('Không thể tải: ' + (err.response?.data?.message || err.message)) } }}>🔒 Tải có Watermark</button></> : <><p>Tài liệu này hiện chưa có URL.</p><p>Instructor có thể cập nhật URL ở trang Quản lý nội dung.</p></>}</div><div className="px-4 py-3 border-t border-border-subtle flex justify-center">{contentStatusMap[normalizeId(getContentId(selectedContent))] === 'COMPLETED' || docRead ? <div className="font-bold text-green-600 text-sm">✅ Đã đọc xong tài liệu</div> : <button type="button" className="px-5 py-2.5 rounded-xl font-bold text-sm border-none cursor-pointer bg-primary-500 text-white transition-all hover:-translate-y-px hover:shadow-[0_4px_16px_rgba(0,86,210,0.25)]" onClick={handleMarkDocRead}>Đánh dấu đã đọc</button>}</div></div>}

            {/* QUIZ */}
            {selectedContent?.contentType === 'QUIZ' && <div className="bg-white border border-border-medium rounded-[18px] overflow-hidden"><div className="flex items-center gap-2.5 px-4 py-3 bg-purple-50 border-b border-border-subtle"><span className="text-xl">✏️</span><h3 className="m-0 text-lg font-bold">Bài kiểm tra</h3></div><div className="px-6 py-10 text-center flex flex-col items-center gap-1.5">{contentStatusMap[normalizeId(getContentId(selectedContent))] === 'COMPLETED' ? <><div className="text-[2.8rem] mb-0.5">✅</div><h4 className="m-0 text-lg font-bold">Đã hoàn thành bài kiểm tra!</h4><p className="m-0 text-text-muted text-[0.88rem]">Bạn có thể làm lại bất cứ lúc nào.</p><button type="button" className="mt-1.5 px-7 py-2.5 rounded-xl font-bold text-sm border-none cursor-pointer bg-primary-500 text-white transition-all hover:-translate-y-px hover:shadow-[0_4px_16px_rgba(0,86,210,0.25)]" onClick={handleGoToQuiz}>Làm lại bài kiểm tra</button></> : <><div className="text-[2.8rem] mb-0.5">📝</div><h4 className="m-0 text-lg font-bold">Sẵn sàng kiểm tra kiến thức?</h4><div className="flex gap-1.5 items-center text-text-muted text-[0.82rem] mb-1"><span>10 phút</span><span>•</span><span>Điểm đạt: 60%</span></div><button type="button" className="mt-1.5 px-7 py-2.5 rounded-xl font-bold text-sm border-none cursor-pointer bg-primary-500 text-white transition-all hover:-translate-y-px hover:shadow-[0_4px_16px_rgba(0,86,210,0.25)]" onClick={handleGoToQuiz}>Bắt đầu làm bài</button></>}</div></div>}

            {/* PRACTICE */}
            {selectedContent?.contentType === 'PRACTICE' && <div className="bg-white border border-border-medium rounded-[18px] p-4"><div className="flex items-center gap-3 mb-4"><span className="text-2xl">💻</span><div><h3 className="m-0 text-lg font-bold">{(practiceSession?.title as string) || 'Bài thực hành'}</h3><p className="m-0 text-text-muted text-sm">{(practiceSession?.language as string) || 'N/A'} • {(practiceSession?.difficulty as string) || 'N/A'}</p></div></div>{practiceLoading && <div className="text-sm text-text-muted py-4">Đang tải bài thực hành...</div>}{!practiceLoading && practiceError && <div className="text-sm text-text-muted py-4">{practiceError}</div>}{!practiceLoading && !practiceError && practiceSession && <div className="grid grid-cols-1 lg:grid-cols-2 gap-4"><div><h4 className="m-0 mb-2 text-base font-bold">Mô tả bài toán</h4><p className="text-text-secondary text-sm leading-relaxed">{(practiceSession.description as string) || 'Chưa có mô tả.'}</p><h4 className="mt-4 mb-2 text-base font-bold">Test case hiển thị</h4><div className="flex flex-col gap-2">{((practiceSession.visibleTestCases as AnyObj[]) || []).length === 0 && <p className="text-text-muted text-sm">Không có test case.</p>}{((practiceSession.visibleTestCases as AnyObj[]) || []).map((tc, idx) => <div key={(tc.testcaseId as string) || idx} className="bg-bg-deep border border-border-subtle rounded-xl px-3 py-2 text-sm"><div className="font-semibold text-text-main mb-1">Case {idx + 1} · {String(tc.outputType)}</div><div><strong>Input:</strong> <code className="bg-gray-100 px-1 rounded">{String(tc.inputData || '(trống)')}</code></div><div><strong>Expected:</strong> <code className="bg-gray-100 px-1 rounded">{String(tc.expectedOutput || '(trống)')}</code></div>{!!tc.description && <div><strong>Mô tả:</strong> {String(tc.description)}</div>}</div>)}</div></div><div><div className="bg-slate-800 text-slate-300 rounded-t-xl px-3 py-2 text-sm font-semibold flex items-center gap-2"><span className="text-lg">⚡</span> Code Editor ({String(practiceSession.language)}) <span className="text-[0.72rem] bg-white/15 px-2 py-0.5 rounded-full">Monaco</span></div><div className="border border-slate-700 rounded-b-xl overflow-hidden" style={{ height: '350px' }}><MonacoEditor height="350px" language={String(practiceSession.language).toLowerCase() === 'python' ? 'python' : 'java'} theme="vs-dark" value={practiceCode} onChange={(v) => setPracticeCode(v || '')} options={{ fontSize: 14, minimap: { enabled: false }, lineNumbers: 'on', scrollBeyondLastLine: false, wordWrap: 'on', automaticLayout: true, tabSize: 4, padding: { top: 8 } }} /></div>{!practiceCode && <p className="text-text-muted text-[0.82rem] mt-1">Chưa có starter code.</p>}<div className="flex gap-2 mt-2"><button type="button" className="px-5 py-2 rounded-xl font-bold text-sm border-none cursor-pointer bg-primary-500 text-white transition-all hover:-translate-y-px" onClick={() => setPracticeCode(extractPracticeStarterCode(practiceSession))}>Reset starter code</button><button type="button" className="px-5 py-2 rounded-xl font-bold text-sm border-none cursor-pointer bg-primary-500 text-white transition-all hover:-translate-y-px disabled:opacity-60" onClick={handleSubmitPractice} disabled={submittingPractice}>{submittingPractice ? 'Đang chấm...' : '▶ Run / Submit'}</button></div>{practiceSubmitError && <p className="text-red-500 text-sm mt-2">{practiceSubmitError}</p>}{practiceResult && <div className="mt-3 border border-border-medium rounded-xl p-3"><div className="font-bold text-sm mb-2"><strong>Kết quả:</strong> Pass {(practiceResult.passed as number) ?? 0} • Fail {(practiceResult.failed as number) ?? 0}</div><div className="flex flex-col gap-2">{((practiceResult.results as AnyObj[]) || []).map((r, idx) => <div key={String(r.testcaseId || idx)} className="bg-bg-deep border border-border-subtle rounded-xl px-3 py-2 text-sm"><div className="font-semibold mb-1">Case {idx + 1} · {r.status === 'PASS' ? '✅ PASS' : '❌ FAIL'} {r.hidden ? '(hidden)' : ''}</div><div><strong>Input:</strong> <code className="bg-gray-100 px-1 rounded">{String(r.inputData || '(trống)')}</code></div><div><strong>Expected:</strong> <code className="bg-gray-100 px-1 rounded">{String(r.expectedOutput || '(trống)')}</code></div><div><strong>Actual:</strong> <code className="bg-gray-100 px-1 rounded">{String(r.actualOutput || '(trống)')}</code></div></div>)}</div></div>}<AIHintsPanel code={practiceCode} language={String(practiceSession.language)} description={String(practiceSession.description || '')} testResults={practiceResult} visible={true} /></div></div>}
            </div>}
          </section>
        </div>
      </main>
