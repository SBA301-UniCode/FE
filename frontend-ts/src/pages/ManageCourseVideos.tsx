...
const ManageCourseVideos = () => {
  ...
  const handleDeleteQB = async (q: AnyObj) => { const qid = (q.questionBankId || q.id) as string; if (!qid || !window.confirm('Xóa câu hỏi này?')) return; try { await questionBankApi.delete(qid); setQuestionBankItems((p) => p.filter((x) => (x.questionBankId || x.id) !== qid)); setActionMsg('Đã xóa câu hỏi.') } catch (err) { setUploadError(getApiErrorMessage(err, 'Xóa thất bại.')) } }
    ...
    <div className="min-h-screen bg-bg-page text-text-main flex flex-col">
      ...
      <main className="w-full mx-auto px-6 py-6 pb-16">
        ...
        {!loadingChapters && <>
          ...
          {selectedLessonId && <>
            ...
            {questionBankItems.length > 0 && <section className={sectionC}>
              <h2 className="m-0 mb-3 text-lg font-bold">5. Ngân hàng câu hỏi ({questionBankItems.length})</h2>
        ...
        {showQuizEditModal && <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-[900]" onClick={() => !savingQuizEdit && setShowQuizEditModal(false)}><div className="bg-white rounded-[18px] border border-border-medium w-[95%] max-w-[480px] max-h-[90vh] overflow-y-auto shadow-[0_8px_32px_rgba(0,0,0,0.12)] px-7 py-6 flex flex-col gap-4" onClick={(e) => e.stopPropagation()}>
          <h2 className="m-0 text-lg font-extrabold">Sửa bài kiểm tra</h2>
          ...
          <div className="grid grid-cols-3 gap-3 max-[640px]:grid-cols-1">
            <label className="flex flex-col gap-1 text-[0.82rem] font-semibold">Thời gian (giây)<input className={inputC} type="number" min={60} value={editQuizForm.duration} onChange={(e) => setEditQuizForm({ ...editQuizForm, duration: Number(e.target.value) })} /></label>
            <label className="flex flex-col gap-1 text-[0.82rem] font-semibold">Điểm đạt (%)<input className={inputC} type="number" min={0} max={100} value={editQuizForm.passScore} onChange={(e) => setEditQuizForm({ ...editQuizForm, passScore: Number(e.target.value) })} /></label>
            <label className="flex flex-col gap-1 text-[0.82rem] font-semibold">Số câu (0=hết)<input className={inputC} type="number" min={0} value={editQuizForm.numberQuestions} onChange={(e) => setEditQuizForm({ ...editQuizForm, numberQuestions: Number(e.target.value) })} /></label>
          </div>
