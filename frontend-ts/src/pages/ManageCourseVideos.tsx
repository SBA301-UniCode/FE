...
const ManageCourseVideos = () => {
  ...
  const handleCreateChapter = async () => { if (!newChapterTitle.trim() || !courseId) return; setCreatingChapter(true); clearMessages(); try { const created = unwrap(await chapterApi.create({ courseId, title: newChapterTitle.trim(), orderIndex: chapters.length })) as AnyObj; if (!created) throw new Error('Không nhận được dữ liệu.'); setNewChapterTitle(''); setActionMsg('Tạo chương thành công!'); const r = await fetchChapters(); setChapters(r || [...chapters, created]); const nId = String(created.chapterId || created.id || ''); if (nId) setSelectedChapterId(nId) } catch (err) { setError(getApiErrorMessage(err, 'Tạo chương thất bại.')) } finally { setCreatingChapter(false) } }
  const handleCreateLesson = async () => { if (!newLessonTitle.trim() || !selectedChapterId) return; setCreatingLesson(true); clearMessages(); try { const created = unwrap(await lessonApi.create({ chapterId: selectedChapterId, title: newLessonTitle.trim(), orderIndex: lessons.length })) as AnyObj; if (!created) throw new Error('Không nhận được dữ liệu.'); setNewLessonTitle(''); setActionMsg('Tạo bài giảng thành công!'); const r = await fetchLessons(selectedChapterId); setLessons(r || [...lessons, created]); const nId = String(created.lessonId || created.id || ''); if (nId) setSelectedLessonId(nId) } catch (err) { setError(getApiErrorMessage(err, 'Tạo bài giảng thất bại.')) } finally { setCreatingLesson(false) } }
  const handleRenameChapter = async () => { if (!selectedChapterId || !chapterEditTitle.trim()) return; setRenamingChapter(true); clearMessages(); try { await chapterApi.update(selectedChapterId, { title: chapterEditTitle.trim() }); setActionMsg('Đã cập nhật tên chương.'); setChapters(await fetchChapters() || []) } catch (err) { setError(getApiErrorMessage(err, 'Cập nhật thất bại.')) } finally { setRenamingChapter(false) } }
  const handleDeleteChapter = async () => { if (!selectedChapterId || !window.confirm('Xóa chương này?')) return; setDeletingChapter(true); clearMessages(); try { await chapterApi.delete(selectedChapterId); setActionMsg('Đã xóa chương.'); setChapters(await fetchChapters() || []); setSelectedChapterId(''); setSelectedLessonId(''); setLessons([]); setContents([]); setVideoMap({}) } catch (err) { setError(getApiErrorMessage(err, 'Xóa thất bại.')) } finally { setDeletingChapter(false) } }
  const handleRenameLesson = async () => { if (!selectedLessonId || !lessonEditTitle.trim()) return; setRenamingLesson(true); clearMessages(); try { await lessonApi.update(selectedLessonId, { title: lessonEditTitle.trim() }); setActionMsg('Đã cập nhật tên bài giảng.'); setLessons(await fetchLessons(selectedChapterId) || []) } catch (err) { setError(getApiErrorMessage(err, 'Cập nhật thất bại.')) } finally { setRenamingLesson(false) } }
  const handleDeleteLesson = async () => { if (!selectedLessonId || !window.confirm('Xóa bài giảng này?')) return; setDeletingLesson(true); clearMessages(); try { await lessonApi.delete(selectedLessonId); setActionMsg('Đã xóa bài.'); const r = await fetchLessons(selectedChapterId); setLessons(r || []); setSelectedLessonId(''); setContents([]); setVideoMap({}); setQuestionBankItems([]) } catch (err: unknown) { const shouldForce = window.confirm(`${getApiErrorMessage(err, 'Xóa thất bại.')}\n\nThử xóa nội dung trước rồi xóa lại?`); if (!shouldForce) { setError(getApiErrorMessage(err, 'Xóa bài thất bại.')); setDeletingLesson(false); return }; try { const lc = await fetchContents(selectedLessonId) || []; for (const ct of lc) await contentApi.delete(ct.contentId as string); await lessonApi.delete(selectedLessonId); setActionMsg('Đã xóa.'); setLessons(await fetchLessons(selectedChapterId) || []); setSelectedLessonId(''); setContents([]); setVideoMap({}); setQuestionBankItems([]) } catch (fe) { setError(getApiErrorMessage(fe, 'Xóa thất bại.')) } } finally { setDeletingLesson(false) } }

  const handleUploadVideo = async (e: React.FormEvent) => { e.preventDefault(); if (!selectedLessonId || !uploadFile) { setUploadError('Chọn bài giảng và file video.'); return }; setUploadProgress(0); setUploading(true); clearMessages(); try { setUploadStep('Bước 1/3: Tạo link upload...'); const presign = unwrap(await videoApi.generateUploadUrl({ fileName: uploadFile.name, contentType: uploadFile.type || 'video/mp4', size: String(uploadFile.size || 0) })) as AnyObj; const uploadUrl = presign?.uploadUrl as string; const key = presign?.key as string; if (!uploadUrl || !key) throw new Error('Backend không trả về uploadUrl/key.'); setUploadStep('Bước 2/3: Đang upload lên S3...'); await uploadFileToS3WithProgress(uploadUrl, uploadFile, setUploadProgress); setUploadStep('Bước 3/3: Đọc thời lượng & lưu...'); const dur = await getVideoDurationSecondsFromFile(uploadFile); const durMin = Math.max(1, Math.round(dur / 60)); const video = unwrap(await videoApi.createVideoRecord({ lessonId: selectedLessonId, duration: durMin, key })) as AnyObj; const cid = video?.contentId as string; setUploadStep(''); setUploadFile(null); setActionMsg('Upload video thành công!'); if (video && cid) setVideoMap((p) => ({ ...p, [cid]: video })); if (cid) setContents((p) => p.some((c) => c.contentId === cid) ? p : [...p, { contentId: cid, contentType: 'VIDEO', lessonId: selectedLessonId }]); const r = await fetchContents(selectedLessonId); if (r) { setContents(r); setVideoMap(await fetchVideoMap(r)) } } catch (err) { setUploadError(getApiErrorMessage(err, 'Upload thất bại.')); setUploadStep('') } finally { setUploading(false); setUploadProgress(0) } }
  const handleCreateDocument = async (e: React.FormEvent) => { e.preventDefault(); if (!selectedLessonId) return; if (!uploadDocFile) { setUploadError('Chọn file tài liệu.'); return }; setCreatingDoc(true); clearMessages(); try { const created = unwrap(await documentApi.create({ lessonId: selectedLessonId, title: documentTitle.trim() || 'Tài liệu bài giảng' }, uploadDocFile)) as AnyObj; const cid = created?.contentId as string; if (!cid) throw new Error('Không nhận được contentId.'); setDocumentTitle(''); setUploadDocFile(null); setActionMsg('Tạo tài liệu thành công!'); if (cid) setContents((p) => p.some((c) => c.contentId === cid) ? p : [...p, { contentId: cid, contentType: 'DOCUMENT', lessonId: selectedLessonId }]); await loadLessonContents(selectedLessonId) } catch (err) { setUploadError(getApiErrorMessage(err, 'Tạo tài liệu thất bại.')) } finally { setCreatingDoc(false) } }
  const handleDeleteContent = async (ct: AnyObj) => { const cid = getContentId(ct); if (!cid) { setUploadError('Không xác định được contentId.'); return }; const label = CONTENT_LABELS[ct.contentType as string] || ct.contentType; if (!window.confirm(`Xóa ${label}?`)) return; clearMessages(); try { if (ct.contentType === 'DOCUMENT') { const doc = documentMap[cid]; if (!doc?.documentId) throw new Error('Không tìm thấy documentId.'); await documentApi.delete(doc.documentId as string) } else { await contentApi.delete(cid) }; const lid = selectedLessonId || (ct.lessonId as string); if (lid) { const nm = { ...readDeletedContentIdMap() }; const cd = Array.isArray(nm[lid]) ? nm[lid] : []; if (!cd.includes(cid)) nm[lid] = [...cd, cid]; writeDeletedContentIdMap(nm); setDeletedContentIdMap(nm) }; const dm = { ...documentMap }; if (dm[cid]) { delete dm[cid]; setDocumentMap(dm) }; setContents((p) => p.filter((i) => getContentId(i) !== cid)); setVideoMap((p) => { const n = { ...p }; delete n[cid]; return n }); setActionMsg(`Đã xóa ${label}`) } catch (err) { setUploadError(getApiErrorMessage(err, 'Xóa thất bại.')) } }
  ...
  const handleCreateQuiz = async () => { if (!selectedLessonId) return; const vQs = quizQuestions.filter((q) => q.text.trim() && q.options.some((o) => o.text.trim())); if (vQs.length === 0 && quizNumQuestions <= 0 && questionBankItems.length === 0) { setUploadError('Cần ít nhất 1 câu hỏi.'); return }; setSavingQuiz(true); clearMessages(); try { if (vQs.length > 0) { setQuizStep(`Lưu ${vQs.length} câu...`); for (const q of vQs) { const opts = q.options.filter((o) => o.text.trim()).map((o) => ({ answerText: o.text.trim(), isCorrect: o.id === q.correctId })); await questionBankApi.createQuestion(selectedLessonId, { questionText: q.text.trim(), imageUrl: null, numberAnswers: opts.length, questionType: q.type, options: opts }) } }; const latest = await fetchQuestionBank(selectedLessonId); const avail = Array.isArray(latest) ? latest.length : 0; const numQ = quizNumQuestions > 0 ? quizNumQuestions : avail; if (avail <= 0) { setUploadError('Chưa có câu hỏi nào.'); setQuizStep(''); return }; if (numQ > avail) { setUploadError(`Ngân hàng có ${avail} câu, đề yêu cầu ${numQ}.`); setQuizStep(''); return }; setQuizStep('Tạo bài kiểm tra...'); await examApi.createExam(selectedLessonId, { name: quizTitle.trim() || 'Bài kiểm tra', duration: quizDuration, passScore: quizPassScore, numberQuestions: numQ }); setActionMsg('Tạo bài kiểm tra thành công!'); setShowQuizEditor(false); setQuizStep(''); const r = await fetchContents(selectedLessonId); if (r) setContents(r); else setUploadError('Đã tạo quiz nhưng chưa tải lại được.'); await fetchQuestionBank(selectedLessonId) } catch (err) { setUploadError(getApiErrorMessage(err, 'Tạo quiz thất bại.')); setQuizStep('') } finally { setSavingQuiz(false) } }
  ...
  if (!canManage) return <div className="min-h-screen bg-bg-page text-text-main"><Header /><main className="max-w-[1200px] mx-auto px-6 py-8"><div className={sectionC}>Trang này dành cho Instructor/Admin.</div><Link to="/my-courses" className={`${btnG} no-underline inline-flex items-center gap-1`}>← My Courses</Link></main></div>

  return (
    <div className="min-h-screen bg-bg-page text-text-main">
      <Header />
      <div className="bg-[linear-gradient(135deg,#312e81_0%,#4338ca_50%,#6366f1_100%)] px-6 py-6 pb-7 text-white"><div className="max-w-[1200px] mx-auto"><div className="flex items-center gap-1.5 text-[0.82rem] mb-2 text-white/65"><Link to="/" className="text-white/85 no-underline hover:underline">Trang chủ</Link><span>/</span><Link to="/my-courses" className="text-white/85 no-underline hover:underline">Khóa học của tôi</Link><span>/</span><span>Nội dung</span></div><h1 className="m-0 text-2xl font-extrabold">🎬 Quản lý nội dung khóa học</h1><p className="mt-1 mb-0 text-white/70 text-sm">Chọn chương → bài giảng → thêm Video, Tài liệu, Bài kiểm tra hoặc Bài thực hành.</p></div></div>

      <main className="max-w-[1200px] mx-auto px-6 py-6 pb-16">
        <div className="flex items-center justify-between mb-4"><div /><Link to="/my-courses" className={`${btnG} no-underline`}>← My Courses</Link></div>
        ...
        {loadingChapters && <div className="p-4 text-center text-text-muted">Đang tải chương...</div>}
        {!loadingChapters && <>
          {/* Step 1: Chapter */}
          <section className={sectionC}><h2 className="m-0 mb-3 text-lg font-bold">1. Chọn hoặc tạo chương</h2>
            {chapters.length > 0 ? <select className={`${inputC} mb-3`} value={selectedChapterId} onChange={(e) => setSelectedChapterId(e.target.value)}><option value="">-- Chọn chương --</option>{chapters.map((c) => <option key={gk(c)} value={gk(c)}>{gt(c)}</option>)}</select> : <p className={hint}>Chưa có chương nào.</p>}
          ...
          {/* Step 2: Lesson */}
          <section className={sectionC}><h2 className="m-0 mb-3 text-lg font-bold">2. Chọn hoặc tạo bài giảng</h2>
            ...
            {selectedChapterId && !loadingLessons && <>
              {lessons.length > 0 ? <select className={`${inputC} mb-3`} value={selectedLessonId} onChange={(e) => setSelectedLessonId(e.target.value)}><option value="">-- Chọn bài giảng --</option>{lessons.map((l) => <option key={lk(l)} value={lk(l)}>{lt(l)}</option>)}</select> : <p className={hint}>Chưa có bài giảng.</p>}
          ...
          {selectedLessonId && <>
            {/* Step 3: Add content */}
            <section className={sectionC}><h2 className="m-0 mb-3 text-lg font-bold">3. Thêm nội dung</h2>
