...
const LANGUAGE_TYPES = ['Java', 'Python', 'JavaScript', 'TypeScript', 'C#', 'C++', 'Go', 'Rust', 'Ruby', 'PHP', 'Swift', 'Kotlin'] as const
interface CourseForm { title: string; description: string; price: number | string; instructorId: string; sylabusId: string; languageType: string; imageFile: File | null }
const EMPTY_FORM: CourseForm = { title: '', description: '', price: 0, instructorId: '', sylabusId: '', languageType: '', imageFile: null }
const DRAFT_KEY = 'unicode_course_form_draft_v1'
const saveDraft = (form: Omit<CourseForm, 'imageFile'>) => { try { localStorage.setItem(DRAFT_KEY, JSON.stringify({ title: form.title, description: form.description, price: form.price, instructorId: form.instructorId, sylabusId: form.sylabusId, languageType: form.languageType })) } catch { /* ignore */ } }
...
const MyCourses = () => {
  ...
  const openEdit = (c: AnyObj) => { setEditingCourse(c); resetPreview(); const img = getCourseImage(c); if (img) { setImagePreview(img); setIsObjectPreview(false) }; setForm({ title: getCourseTitle(c), description: getCourseDesc(c), price: (c.price ?? 0) as number, instructorId: (c.instructorId || user?.userId || '') as string, sylabusId: (c.sylabusId || '') as string, languageType: (c.languageType || '') as string, imageFile: null }); setFormError(''); setShowForm(true) }
    ...
    <div className="min-h-screen bg-bg-page text-text-main">
      ...
      <main className="max-w-[1280px] mx-auto px-6 py-6 pb-16">
        ...
        {showForm && <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-[900]" onClick={closeForm}><form className="bg-white border border-border-medium rounded-[18px] px-7 py-6 w-[95%] max-w-[520px] max-h-[90vh] overflow-y-auto flex flex-col gap-4 shadow-[0_8px_32px_rgba(0,0,0,0.12)]" onClick={(ev) => ev.stopPropagation()} onSubmit={handleSubmit}>
          ...
          <label className="flex flex-col gap-1 text-sm font-semibold">{t('myCourses.descriptionLabel')}<textarea className={`${inputCls} resize-y min-h-[220px]`} rows={10} value={form.description} onChange={(ev) => setForm({ ...form, description: ev.target.value })} /></label>
          <label className="flex flex-col gap-1 text-sm font-semibold">{t('myCourses.languageLabel')}<select className={inputCls} value={form.languageType} onChange={(ev) => setForm({ ...form, languageType: ev.target.value })}><option value="">{t('myCourses.languagePlaceholder')}</option>{LANGUAGE_TYPES.map((lang) => <option key={lang} value={lang}>{lang}</option>)}</select></label>
