...
const PaymentSuccess = () => {
  ...
  return (
    <div className="min-h-screen bg-bg-page text-text-main relative overflow-hidden flex flex-col">
      <Header />
      <main className="w-full max-w-[960px] mx-auto px-4 sm:px-6 py-10 pb-16 relative z-[1]">
        {/* Confetti */}
        {!isError && (
        ...
        <div
          className={`w-full max-w-[760px] mx-auto bg-white rounded-3xl p-6 sm:p-10 text-center shadow-[0_8px_28px_rgba(2,6,23,0.08)] animate-[cardPop_0.5s_ease-out] ${isError ? 'border border-red-200' : 'border border-emerald-200'}`}
          style={{ width: 'min(760px, 100%)' }}
        >
          {isError ? (
            <>
              <div className="w-20 h-20 mx-auto mb-5 rounded-full bg-red-50 border-[3px] border-red-500 text-red-600 flex items-center justify-center animate-[iconPulse_0.6s_ease]">
              ...
              <h1 className="m-0 mb-3 text-2xl sm:text-3xl font-extrabold">{t('payment.failedTitle')}</h1>
              <p className="m-0 mb-4 text-text-secondary leading-relaxed text-[0.96rem] sm:text-base">{message || t('payment.failedDesc')}</p>
            ...
            <>
              ...
              <h1 className="m-0 mb-3 text-2xl sm:text-3xl font-extrabold">{t('payment.successTitle')}</h1>
              <p className="m-0 mb-4 text-text-secondary leading-relaxed text-[0.96rem] sm:text-base">{t('payment.successDesc')}</p>
              {enrolling && <p className="text-[0.88rem] text-primary-500 m-0 mb-4">{t('payment.enrolling')}</p>}
              <div className="text-left mb-6 p-4 sm:p-5 bg-gradient-to-br from-emerald-50 to-green-50 rounded-2xl border border-green-200">
