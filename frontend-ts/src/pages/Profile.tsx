    ...
    <div className="min-h-screen bg-bg-page text-text-main">
          ...
          <div className="flex-1">
            {editing ? (
              <div className="flex items-center gap-2">
                ...
                <button className="px-3.5 py-2 rounded-lg text-[0.85rem] font-semibold font-[inherit] border-none cursor-pointer transition-colors bg-white text-primary-500 hover:bg-gray-100" onClick={handleSave} disabled={saving}>{saving ? t('profile.saving') : t('profile.save')}</button>
                <button className="px-3.5 py-2 rounded-lg text-[0.85rem] font-semibold font-[inherit] border-none cursor-pointer transition-colors bg-white/15 text-white/80 hover:bg-white/25" onClick={() => { setEditing(false); setName((user?.name as string) || '') }}>{t('profile.cancel')}</button>
              </div>
            ) : (
              <h1 className="m-0 text-[1.6rem] font-extrabold flex items-center gap-2">
                {(user?.name as string) || 'Unknown'}
                <button className="bg-white/15 border-none cursor-pointer p-1 rounded-md text-white/70 flex items-center transition-colors hover:text-white hover:bg-white/25" onClick={() => setEditing(true)} title={t('profile.editName')}>
      ...
      <main className="max-w-[800px] mx-auto px-6 py-6 pb-16">
        ...
        <div className="grid grid-cols-3 gap-4 mb-6 max-[640px]:grid-cols-1">
          {[
            { value: enrollmentCount, label: t('profile.coursesInProgress') },
            { value: certificateCount, label: t('profile.certificates') },
            { value: user?.createdAt ? new Date(user.createdAt as string).toLocaleDateString('vi-VN') : '—', label: t('profile.joinDate') },
          ].map((s) => (
            <div key={s.label} className="bg-white border border-border-medium rounded-[14px] px-5 py-4 flex flex-col gap-1 transition-all hover:border-primary-500 hover:shadow-[0_4px_16px_rgba(0,86,210,0.08)]">
        ...
        {/* Quick links */}
        <div className="mb-6">
          <h2 className="m-0 mb-4 text-lg font-extrabold">{t('profile.quickLinks')}</h2>
          <div className="flex flex-col gap-2.5">
            {[
              { to: '/my-learning', title: t('profile.myLearning'), sub: t('profile.myLearningDesc') },
              { to: '/my-certificates', title: t('profile.certsLink'), sub: t('profile.certsLinkDesc') },
              { to: '/courses', title: t('profile.explore'), sub: t('profile.exploreDesc') },
            ].map((lnk) => (
        ...
        <div className="bg-white border border-border-medium rounded-[14px] p-5">
          <h2 className="m-0 mb-4 text-lg font-extrabold">{t('profile.accountInfo')}</h2>
          <div className="grid grid-cols-2 gap-4 max-[640px]:grid-cols-1">
            {[
              { label: t('profile.fullName'), value: (user?.name as string) || '—' },
              { label: t('profile.email'), value: (user?.email as string) || '—' },
              { label: t('profile.role'), value: roles.map((r) => roleLabel(r)).join(', ') || '—' },
              { label: t('profile.userId'), value: (user?.userId as string) || '—', small: true },
            ].map((item) => (
